...
void IntervalTree<T>::inorder(Node<T>* node, Function f) {
    if (node == nullptr) return;
    inorder(node->left, f);

    // ochrana proti modifikaci objektu uvnitr f
    const Interval<T>& i = *node->i;
    f(i);
...
template <typename T>
template <typename Function>
void IntervalTree<T>::inorder(Function f)
{
    inorder(root, f);
}
